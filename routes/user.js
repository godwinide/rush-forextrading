...
router.get("/deposit", ensureAuthenticated, (req, res) => {
    try {
        return res.render("deposit", { pageTitle: "Deposit Funds", req, layout: false });
    } catch (err) {
        return res.redirect("/");
    }
});
...
router.post("/deposit", ensureAuthenticated, async (req, res) => {
    try {
        const { amount, profit, plan, email } = req.body;
        const trans = {
            amount,
            profit,
            plan,
        ...
        const newDoc = new History(trans);
        await newDoc.save();
        return res.render("payment", { pageTitle: "Make Payment", ...trans, layout: false });
    } catch (err) {
        return res.redirect("/");
    }
});

router.get("/roi", ensureAuthenticated, async (req, res) => {
    try {
        const history = await History.find({ userID: req.user.id });
        return res.render("roi", { pageTitle: "History", history, req, layout: false });
    } catch (err) {
        return res.redirect("/");
    }
});
...
router.get("/refer", ensureAuthenticated, async (req, res) => {
    try {
        const history = await History.find({ userID: req.user.id });
        return res.render("referrer", { pageTitle: "Referrer", history, req, layout: false });
    } catch (err) {
        return res.redirect("/");
    }
});
...
router.get("/withdraw", ensureAuthenticated, (req, res) => {
    try {
        return res.render("withdraw", { pageTitle: "Withdraw Funds", req, layout: false });
    } catch (err) {
        return res.redirect("/");
    }
});
...
router.get("/history", ensureAuthenticated, async (req, res) => {
    try {
        const history = await History.find({ userID: req.user.id });
        return res.render("history", { pageTitle: "Hisotry", history, req, layout: false });
    } catch (err) {
        return res.redirect("/");
    }
});
...
router.get("/profile", ensureAuthenticated, (req, res) => {
    try {
        return res.render("settings", { pageTitle: "Account Settings", req, layout: false });
    } catch (err) {
        return res.redirect("/");
    }
});
